...
def main() -> None:
  ...
  const FALLBACK_RELEASES = [
    ...
    if count != 1:
        raise RuntimeError("Could not replace site fallback releases")
    old_title = '''    const title = artist && releaseTitle
      ? `${artist} – ${releaseTitle}`
      : (releaseTitle || "Latest NextGen Sessions release");'''
    new_title = '''    const hasArtistPrefix = artist && releaseTitle.toLowerCase().startsWith(artist.toLowerCase());
    const title = artist && releaseTitle && !hasArtistPrefix
      ? `${artist} – ${releaseTitle}`
      : (releaseTitle || "Latest NextGen Sessions release");'''
    if old_title not in text:
        raise RuntimeError("Could not update homepage title normalisation")
    text = text.replace(old_title, new_title, 1)
    text = text.replace('/api/latest?v=6', '/api/latest?v=7')
    text = text.replace('/releases.json?homepage=20260805', '/releases.json?homepage=20260805b')
    site.write_text(text, encoding="utf-8")

    index = ROOT / "index.html"
    html = index.read_text(encoding="utf-8")
    html = html.replace("uSlZrZJompg", "xicnIGw-ei8")
    html = html.replace("Zara Veli – Don’t Call Past 2", "Alia Bleu – Piggyback")
    html = html.replace("Don’t Call Past 2 thumbnail", "Piggyback thumbnail")
    html = html.replace("<h3>Don’t Call Past 2</h3>", "<h3>Piggyback</h3>")
    html = html.replace("site.js?v=20260805-latest1", "site.js?v=20260805-latest2")
    index.write_text(html, encoding="utf-8")

    required = {
        ROOT / "functions/api/latest.js": ["fetchChannelPage(catalogueItems)", 'latest?v=7', 'channel-page'],
        ROOT / "site.js": ["hasArtistPrefix", '/api/latest?v=7', "Alia Bleu – Piggyback"],
        ROOT / "index.html": ["Alia Bleu – Piggyback", "site.js?v=20260805-latest2"],
    }
