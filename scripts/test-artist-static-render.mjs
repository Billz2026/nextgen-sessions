...
for (const artist of artists) {
  ...
  assert.ok(page.includes(`<h1 class="profile-title">${artist.name}</h1>`), `${artist.name} must expose its H1 in HTML`);
  assert.ok(page.includes(profile.headline), `${artist.name} must expose its headline in HTML`);
  ...
  if (published.length) {
    ...
    assert.ok(page.includes(latest.title), `${artist.name} must expose its latest catalogue release in HTML`);
    assert.ok(page.includes(latest.url), `${artist.name} must link to its latest release page in HTML`);
  }
}

console.log(`Static artist render passed: ${artists.length} artist pages expose crawlable profile, biography and discography content.`);
