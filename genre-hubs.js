(function () {
  ...
  function releaseCard(item) {
    ...
    return `<a class="genre-release-card" href="${url}" data-video-id="${esc(item.id)}">
      <div class="genre-release-art"><img loading="lazy" decoding="async" src="/api/release-image?id=${id}&amp;size=card" alt="${title} by ${artist}"></div>
      <div class="genre-release-body"><span class="tag">${group}</span><h3>${title}</h3><p>${artist}</p><span class="genre-release-date">${esc(formatDate(item.published))}</span></div>
    </a>`;
  }
  ...
  function artistCard(artist) {
    ...
    const imageMarkup = hasImage
      ...
      : "";
    return `<a class="genre-artist-card${hasImage ? "" : " no-image"}" href="/artists/${esc(artist.slug)}/">
      ${imageMarkup}
      <div class="genre-artist-copy"><span>${esc(artist.genre)}</span><h3>${esc(artist.name)}</h3><p>${esc(artist.summary)}</p></div>
    </a>`;
  }

  function updateIndex(releases) {
    Object.entries(LANES).forEach(([slug, lane]) => {
      ...
      document.querySelectorAll(`[data-lane-count="${slug}"]`).forEach(node => {
        node.textContent = `${laneItems.length} releases · ${lanePeople.length} artists`;
      });
    ...
    if (releaseTotal) releaseTotal.textContent = String(releases.length);
    if (artistTotal) artistTotal.textContent = String(artists.length);
  }

  function renderHub(releases) {
    ...
    if (releaseCount) releaseCount.textContent = String(laneItems.length);
    ...
    if (releaseGrid) {
      releaseGrid.innerHTML = laneItems.length
        ? laneItems.slice(0, 8).map(releaseCard).join("")
        : '<p class="genre-loading">No releases are currently published in this lane.</p>';
    }

    const artistGrid = document.querySelector("[data-genre-artist-grid]");
    if (artistGrid) {
      artistGrid.innerHTML = lanePeople.length
        ? lanePeople.map(artistCard).join("")
        : '<p class="genre-loading">Artist profiles for this lane are being prepared.</p>';
    }

    const relatedGrid = document.querySelector("[data-related-genres]");
    if (relatedGrid) {
      relatedGrid.innerHTML = Object.entries(LANES)
        .filter(([key]) => key !== slug)
        .slice(0, 3)
        .map(([key, item]) => `<a class="genre-related-card" href="/genres/${key}/"><span>Genre hub</span><strong>${esc(item.name)}</strong></a>`)
        .join("");
    }
  }
